...
# Create MLflow startup script - using different delimiter to avoid conflicts
cat > /opt/mlflow/start_mlflow.sh << 'STARTUP_EOF'
#!/bin/bash
...
export DB_CONNECTION_STRING="$$(echo "$$DB_TEMPLATE" | sed "s/PASSWORD/$$DB_PASSWORD/g")"
